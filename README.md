...
python classify_sentiment.py \
  -i inputs/sample_comments.jsonl \
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `-i, --input` | 输入JSONL文件路径 | 必需 |
| `-o, --output` | 输出JSON文件路径 | 必需 |
| `--comment-field` | JSONL中评论字段名 | `comment` |
...
JSONL 文件需要包含评论字段（默认为 `comment`），示例：

```jsonl
{"comment":"塞尔达传说王国之泪太好玩了！画面精美，玩法创新，强烈推荐！","user":"user_001","date":"2024-05-15"}
{"comment":"这代宝可梦优化太差了，帧数掉得厉害，体验很糟糕","user":"user_002","date":"2024-03-20"}
```
